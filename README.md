...
| 現在の状態 | 0.6.1 alpha、DB 貼り付け整形・CSV 用例作成・単用例出力・選択出力・用例削除・復元・複数画像・連続撮影・動画添付、Windows ビルド検証済み | 0.2 alpha、Mac の実ブラウザで一連の操作を確認済み |
...
- `Ctrl+S`：保存。
- DB / Excel の列名付き CSV・TSV を貼り付けると、追加画面で自動整形・表プレビュー。`00012` やセル内改行を保ち、表として Excel に出力します。
- 「CSV から用例作成…」で用例・操作・テスト条件・期待結果をまとめて作成。[CSV テンプレート](native-windows/examples/case-template.csv) は 1 行＝1 ステップ、同じ用例 ID を自動でまとめます。プレビューと ID 検証後に作成し、既存の用例は上書きしません。
- 「成果物を出力」で「現在の用例のみ」または「全用例」を選択できます。用例選択中は現在の 1 件が初期選択され、Excel・添付・ZIP が同じ範囲になります。
...
- 「＋ 画像（複数）」で 1 件に複数画像。「画像を追加・整理」で追加・並べ替え・個別注釈。同じ Step の既存行は複数選択し「選択を1件にまとめる」で統合できます。連続撮影も同じ行に追加します。複数画像プロジェクトは Desktop 0.3 以降専用で、ブラウザでは開けません。
...
5. **保存・出力**：編集内容を確定し、Excel と対象の添付を ZIP で渡します。
...
| Windows 原生版 | 111 Core checks、x64 ビルド、64 枚画像出力、64 MiB 添付、ステップ条件、Open XML / ExcelJS 検証 | Windows ウィンドウ、RDP、IME、DPI、プレーヤー連携、実 Excel の印刷・クリック |
