...
**Windows alpha 0.6：CSV 批量创建用例、单个用例导出。**

- CSV 批量创建：「CSV から用例作成…」→ 选文件 → 预览 →「プレビューの用例を作成」。一行一个步骤，相同用例 ID 自动归组；支持操作、测试条件和期待结果。
- 单个用例导出：选中用例 →「成果物を出力」→「現在の用例のみ」。也可改选「全用例」，摘要、工作表和附件 ZIP 同步使用所选范围。
...
1. 点击左侧「CSV から用例作成…」→「テンプレートを保存…」，或使用安装包根目录的 `case-template.csv`（[仓库模板](native-windows/examples/case-template.csv)）。
2. 在 Excel 中批量填写，保存为 **CSV UTF-8（含列名）**。不要改成 XLSX；每一行是一个测试步骤。
3. 点击「CSV / TSV を選択…」，预览合并后的用例列表、各步骤和「用例の情報」。
4. 确认后点「プレビューの用例を作成」。全部校验通过才开始创建；完成后可继续编辑、加截图，或只导出当前用例。

| 模板列 | 填写方式 |
| --- | --- |
| 用例ID | 每行必填，例如 TC-CSV001；同一用例的多行使用相同 ID |
| 用例名 | 同一用例至少填写一次，其他行可空白或填写相同名称 |
| ステップ番号 | 可空白，按用例自动编号；手填时必须为不重复的正整数 |
| 操作 | 每行必填 |
| テスト条件 | 可空白，可在单元格中换行 |
| 期待結果 | 列必须保留，内容可先空白，支持多行 |

例如 TC-CSV001 连续写 3 行不同操作，导入后就是 **1 个用例、3 个步骤**。CSV 的逗号、双引号、换行由 Excel 正常保存即可，不必手动拼字符串。同一 ID 的行不相邻也能合并，保留首次出现的用例顺序和各步骤的行顺序。

支持 UTF-8 CSV / TSV、日文模板列名及英文 `caseId,title,stepNo,action,condition,expected`，也支持常见中文列名。可追加 `実際結果 / actual`、`判定 / verdict`、`前提条件 / precondition`、`担当者 / tester`、`実施日 / date`、`環境 / env`、`用例備考 / caseNote`。用例共通信息的非空值必须一致；担当者和环境缺省使用项目设置，日期缺省使用导入当天。判定留空表示未填写。空白步骤号从 1 起；混合手动编号时，后续空白号使用此前最大编号 + 1。

上限为 2 MiB、10,000 条步骤记录。格式错误、重复步骤号、名称冲突、已有/已删除用例 ID 等会阻止整批创建，不覆盖已有数据。磁盘或权限错误如果发生在创建途中，已成功的用例会保留，窗口中留下未完成部分供重试；这不是跨全部用例的事务。强制结束后，重新导入时需先从 CSV 移除已创建用例的行，避免重复 ID。

用例 CSV 用于批量创建测试定义；DB 查询结果仍从「＋ 証拠」添加为表格证据。两种入口各有用途。
...
- 原生版：104 项核心检查、64 张图片导出、64 MiB 视频附件分段处理、步骤条件读写与导出、Windows x64 编译、Open XML / ExcelJS 验证。
