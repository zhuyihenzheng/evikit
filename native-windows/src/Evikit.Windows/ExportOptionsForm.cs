...
internal sealed class ExportOptionsForm : Form
{
    ...
    private readonly RadioButton currentOnly;
    private readonly string? currentCaseId;
    public string? CaseId => currentOnly.Checked ? currentCaseId : null;
    ...
    public ExportOptionsForm(ExportOptions options, string? currentCaseId = null, string? currentCaseTitle = null)
    {
        this.currentCaseId = currentCaseId;
        Text = "成果物の出力設定"; Font = SystemFonts.MessageBoxFont; AutoScaleMode = AutoScaleMode.Dpi;
        ClientSize = new(660, 720); MinimumSize = new(580, 490); StartPosition = FormStartPosition.CenterParent;
        ...
        Label("出力範囲");
        currentOnly = new RadioButton { Text = currentCaseId == null ? "現在の用例のみ（用例を選択してから出力してください）" : "現在の用例のみ：" + currentCaseId, AutoSize = true, Enabled = currentCaseId != null, Checked = currentCaseId != null, Margin = new(8, 5, 8, 5) };
        var allCases = new RadioButton { Text = "全用例（検索・NG フィルターに関係なくプロジェクト全体）", AutoSize = true, Checked = currentCaseId == null, Margin = new(8, 5, 8, 5) };
        content.Controls.Add(currentOnly); content.Controls.Add(allCases);
        if (currentCaseId != null) Label("現在の用例：" + currentCaseId + " — " + currentCaseTitle);
        ...
        Label("出力項目はプロジェクトごとに記憶します。出力範囲は毎回選択します。対象用例の操作・結果・判定、表とログの本文は常に出力します。本文・画像内の日時や文字の自動削除は行いません。");
        var next = Ui.Button("出力先を選ぶ…", () => DialogResult = DialogResult.OK, true);
        var cancel = Ui.Button("キャンセル", () => DialogResult = DialogResult.Cancel);
        var reset = Ui.Button("全項目をオン", () => { foreach (var box in content.Controls.OfType<CheckBox>()) box.Checked = true; });
